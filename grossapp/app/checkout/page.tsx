...
import { CreditCardIcon, LocateFixed, Truck } from 'lucide-react'
...
export default function Checkout() {
  ...
  const data: any = useSelector((state: RootState) => state.user.userData)
 ...
 useEffect(() => {
  const userId = session?.user?.id || user?._id
  if (!userId) return
  ...
  useEffect(() => {
    ...
    axios
      .get('https://nominatim.openstreetmap.org/reverse', {
        params: { format: 'json', lat: location[0], lon: location[1] },
      })
      .then(res => {
        setAddress(prev => ({
          ...prev,
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if(!socketRef.current.connected){
        return toast.error("s not connect")
      }
        ...
        socketRef.current.emit("manage-orders",orderItems)
  ...
  const paymentOnline = async () => {
    ...
    try {
      const { data: res } = await axios.post('/api/user/payment', {
        userId: data._id,
        items: orderItems,
            ...
            <div className="h-72 rounded overflow-hidden">
              {location && (
                <MapContainer center={location} zoom={16} className="w-full h-full">
                  ...
                  <Marker position={location} draggable eventHandlers={{
                    dragend: e => {
